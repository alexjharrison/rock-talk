...
CREATE TRIGGER set_public_channel_groups_updated_at BEFORE UPDATE ON public.channel_groups FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
COMMENT ON TRIGGER set_public_channel_groups_updated_at ON public.channel_groups IS 'trigger to set value of column "updated_at" to current timestamp on row update';
...
CREATE TRIGGER set_public_channel_users_updated_at BEFORE UPDATE ON public.channel_users FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
COMMENT ON TRIGGER set_public_channel_users_updated_at ON public.channel_users IS 'trigger to set value of column "updated_at" to current timestamp on row update';
...
ALTER TABLE ONLY public.channel_groups
    ADD CONSTRAINT channel_groups_channel_id_fkey FOREIGN KEY (channel_id) REFERENCES public.channel(id);
ALTER TABLE ONLY public.channel_groups
    ADD CONSTRAINT channel_groups_group_id_fkey FOREIGN KEY (group_id) REFERENCES public."group"(id) ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE ONLY public.channel
    ADD CONSTRAINT channel_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES auth.users(id);
ALTER TABLE ONLY public.channel_users
    ADD CONSTRAINT channel_users_channel_id_fkey FOREIGN KEY (channel_id) REFERENCES public.channel(id);
ALTER TABLE ONLY public.channel_users
    ADD CONSTRAINT channel_users_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id);
ALTER TABLE ONLY public.equipment
